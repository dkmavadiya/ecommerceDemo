...
public class DemandDaoImpl implements DemandDao {
	...
	public boolean addProduct(String userId, String prodId, int demandQty) {
		...
		try {
			ps = con.prepareStatement("select * from dbo.[user_demand] where username=? and prodid=?");
			...
			if (rs.next()) {
				...
				ps2 = con.prepareStatement("insert into  dbo.[user_demand] values(?,?,?)");
	...
	public boolean removeProduct(String userId, String prodId) {
		...
		try {
			...
			if (rs.next()) {
				...
				ps2 = con.prepareStatement("delete from  dbo.[user_demand] where username=? and prodid=?");
	...
	public List<DemandBean> haveDemanded(String prodId) {
		...
		try {
			ps = con.prepareStatement("select * from dbo.[user_demand] where prodid=?");
