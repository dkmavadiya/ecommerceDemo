...
public class ProductDaoImpl implements ProductDao {
	...
	public String addProduct(ProductBean product) {
		...
		String validateProductData = validateProductData(product);
		...
		try {
			ps = con.prepareStatement("insert into dbo.[product] values(?,?,?,?,?,?,?);");
	...
	public String removeProduct(String prodId) {
		...
		try {
			ps = con.prepareStatement("delete from dbo.[product] where pid=?");
			...
			if (k > 0) {
				...
				ps2 = con.prepareStatement("delete from dbo.[usercart] where prodid=?");
	...
	public String updateProduct(ProductBean prevProduct, ProductBean updatedProduct) {
		...
		String validateProductData = validateProductData(updatedProduct);
		if (validateProductData != null) {
			return validateProductData;
		}
		...
		try {
			ps = con.prepareStatement(
					"update dbo.[product] set pname=?,ptype=?,pinfo=?,pprice=?,pquantity=?,image=? where pid=?");
	...
	public String updateProductPrice(String prodId, double updatedPrice) {
		...
		try {
			ps = con.prepareStatement("update dbo.[product] set pprice=? where pid=?");
	...
	public List<ProductBean> getAllProducts() {
		...
		try {
			ps = con.prepareStatement("select * from dbo.[product]");
	...
	public byte[] getImage(String prodId) {
		...
		try {
			ps = con.prepareStatement("select image from dbo.[product] where  pid=?");
	...
	public ProductBean getProductDetails(String prodId) {
		...
		try {
			ps = con.prepareStatement("select * from dbo.[product] where pid=?");
	...
	public String updateProductWithoutImage(String prevProductId, ProductBean updatedProduct) {
		...
		try {
			ps = con.prepareStatement(
					"update dbo.[product] set pname=?,ptype=?,pinfo=?,pprice=?,pquantity=? where pid=?");
	...
	public boolean sellNProduct(String prodId, int n) {
		...
		try {
			...
			ps = con.prepareStatement("update dbo.[product] set pquantity=(pquantity - ?) where pid=?");
	 ...
	 * @return
	 */
	private String validateProductData(ProductBean product) {
		StringBuilder validate = new StringBuilder();
		if (product.getProdName() == null || product.getProdName().trim().isEmpty()) {
			validate.append("Product name is required");
		}

		if (product.getProdType() == null || product.getProdType().trim().isEmpty()) {
			validate.append("Product type is required");
		}

		if (product.getProdPrice() == 0) {
			validate.append("Product price is required");
		} else {
			if (product.getProdPrice() <= 0) {
				validate.append("Product price is invalid");
			}
		}

		if (product.getProdQuantity() == 0) {
			validate.append("Product Quantity is required");
		} else {
			if (product.getProdQuantity() <= 0) {
				validate.append("Product Quantity is invalid");
			}
		}

		if (product.getProdImage() == null) {
			validate.append("Product Image is required");
		}

		return validate.toString().trim().isEmpty() ? null : validate.toString();
	}
}
