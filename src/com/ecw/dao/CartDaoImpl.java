...
public class CartDaoImpl implements CartDao {
	...
	public String addProductToCart(String userId, String prodId, int prodQty) {
		...
		try {
			...
			ps = con.prepareStatement("select * from dbo.[usercart] where username=? and prodid=?");
	...
	public List<CartBean> getAllCartItems(String userId) {
		...
		try {
			...
			ps = con.prepareStatement("select * from dbo.[usercart] where username=?");
	...
	public int getCartCount(String userId) {
		...
		try {
			ps = con.prepareStatement("select sum(quantity) from dbo.[usercart] where username=?");
	...
	public String removeProductFromCart(String userId, String prodId) {
		...
		try {
			...
			if (rs.next()) {
				...
				if (prodQuantity > 0) {
					ps2 = con.prepareStatement("update dbo.[usercart] set quantity=? where username=? and prodid=?");
					...
					ps2 = con.prepareStatement("delete from dbo.[usercart] where username=? and prodid=?");
	...
	public boolean removeAProduct(String userId, String prodId) {
		...
		try {
			...
			ps = con.prepareStatement("delete from dbo.[usercart] where username=? and prodid=?");
			ps.setString(1, userId);
			ps.setString(2, prodId);
	...
	public String updateProductToCart(String userId, String prodId, int prodQty) {
		...
		try {
			...
			if (rs.next()) {
				...
				ps2 = con.prepareStatement("insert into dbo.[usercart] values(?,?,?)");
	...
	public int getProductCount(String userId, String prodId) {
		...
		try {
			ps = con.prepareStatement("select sum(quantity) from dbo.[usercart] where username=? and prodid=?");
			ps.setString(1, userId);
			ps.setString(2, prodId);
			rs = ps.executeQuery();
